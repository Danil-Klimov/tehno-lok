		...
		<div class="row">
			...
			<div class="video-block__container video-border">
						...
						<?php if (get_sub_field('video-block_online')) : ?>
							<a class="video__link" href="<?php the_sub_field('video-block_online'); ?>">
								...
								<?php if ($video_block_cover) : ?>
									<picture>
										<source srcset="<?php echo esc_url($video_block_cover['sizes']['515x290']); ?>"
														media="(max-width: 768px)">
										<source srcset="<?php echo esc_url($video_block_cover['sizes']['695x390']); ?>"
														media="(max-width: 992px)">
										<source srcset="<?php echo esc_url($video_block_cover['sizes']['935x525']); ?>"
														media="(max-width: 1200px)">
										<img class="video__media"
												 src="<?php echo esc_url($video_block_cover['sizes']['630x355']); ?>"
												 alt="<?php echo esc_attr($video_block_cover['alt']); ?>"
												 width="<?php echo $video_block_cover['sizes']['935x525-width']; ?>"
												 height="<?php echo $video_block_cover['sizes']['935x525-height']; ?>">
									</picture>
								<?php endif; ?>
							</a>
						<?php else : ?>
				...
				<?php else : ?>
					...
					<?php if ($video_block_local) : ?>
						<video src="<?php echo esc_url($video_block_local['url']); ?>" controls></video>
					<?php endif; ?>
				<?php endif; ?>
			</div>
			<div class="video-block__text">
				...
				<button class="button button_fill button_w-icon" type="button" data-src="#modal-request" data-fancybox>
					<img src="<?php echo get_template_directory_uri() ?>/assets/images/request-icon.png" alt="">
