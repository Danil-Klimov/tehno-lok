			...
			<?php if (have_rows('team_tabs')) : ?>
				<div class="team__tabs">
					<?php while (have_rows('team_tabs')) : the_row(); ?>
						<a class="team__tab<?php echo get_row_index() === 0 ? ' active' : '' ?>"
							 href="#tab_<?php echo get_row_index(); ?>"><?php the_sub_field('tab-name'); ?></a>
					<?php endwhile; ?>
				</div>
			<?php endif; ?>
			<?php if (have_rows('team_tabs')) : ?>
				<?php while (have_rows('team_tabs')) : the_row(); ?>
					<div class="team__container<?php echo get_row_index() === 0 ? ' active' : '' ?>"
							 id="tab_<?php echo get_row_index(); ?>">
									...
									<?php while (have_rows('staff')) : the_row(); ?>
										<div class="swiper-slide">
											<div class="team__item">
												...
												<?php if ($img) : ?>
													<img src="<?php echo esc_url($img['sizes']['210x270']); ?>"
															 alt="<?php echo esc_attr($img['alt']); ?>"
															 width="<?php echo $img['sizes']['210x270-width']; ?>"
															 height="<?php echo $img['sizes']['210x270-height']; ?>"/>
												<?php endif; ?>
