					...
					<?php while (have_rows('serv-menu_blocks')) : the_row(); ?>
						...
						<?php if (get_row_index() === 0) : ?>
							<div class="serv-menu__carcas" style="background-image: url(<?php the_sub_field('bg'); ?>)">
								...
								<?php else : ?>
									<a class="serv-menu__title serv-menu__carcas-title" href="<?php echo esc_url($title_link['url']); ?>"
										<?php echo get_sub_field('type-title') === 'popup' ? 'data-fancybox' : '' ?>>
										<?php echo esc_html($title_link['title']); ?>
									</a>
								<?php endif; ?>
								<?php if (have_rows('anchors')) : ?>
									<?php while (have_rows('anchors')) : the_row(); ?>
										...
										<?php else : ?>
											<a class="serv-menu__link" href="<?php echo esc_url($anchor['url']); ?>"
												<?php echo get_sub_field('anchor-type') === 'popup' ? 'data-fancybox' : '' ?>>
												<span><?php echo esc_html($anchor['title']); ?></span>
											</a>
										<?php endif; ?>
									<?php endwhile; ?>
								<?php endif; ?>
								...
								<div class="serv-menu__coating-footer">
									...
									<?php else : ?>
										<a class="serv-menu__title serv-menu__coating-title"
											 href="<?php echo esc_url($title_link['url']); ?>"
											<?php echo get_sub_field('type-title') === 'popup' ? 'data-fancybox' : '' ?>>
											<?php echo esc_html($title_link['title']); ?>
										</a>
									<?php endif; ?>
